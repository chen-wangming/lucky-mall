...
public interface UserMapper {
     ...
     */
    @Insert("insert into t_user(username,password,phone,email,score,status,image,nickname) " +
            "values(#{user.username},#{user.password},#{user.phone},#{user.email},#{user.score},#{user.status},#{user.image},#{user.nickname})")
     ...
     */
    @Update("update t_user set username=#{username},password=#{password},phone=#{phone},email=#{email},score=#{score}," +
            "status=#{status},image=#{image},nickname=#{nickname} where id=#{id}")
