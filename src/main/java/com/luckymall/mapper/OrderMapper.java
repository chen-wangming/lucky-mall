...
public interface OrderMapper {
     ...
     */
    @Insert("insert into t_order(order_code,create_time,status,total_price,user_id) " +
            "values(#{order.orderCode},#{order.createTime},#{order.status},#{order.totalPrice},#{order.userId})")
     ...
     */
    @Update("update t_order set order_code=#{orderCode},create_time=#{createTime},status=#{status}," +
            "total_price=#{totalPrice},user_id=#{userId} where id=#{id}")
